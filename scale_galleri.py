...
with open("app/galleri/page.tsx", "r") as f:
    lines = f.readlines()

STATIC_MAPPINGS = {
    "max-w-7xl": "max-w-6xl 2xl:max-w-7xl",
    "max-w-6xl": "max-w-5xl 2xl:max-w-6xl",
    "max-w-5xl": "max-w-4xl 2xl:max-w-5xl",
    "max-w-4xl": "max-w-3xl 2xl:max-w-4xl",
    "max-w-3xl": "max-w-2xl 2xl:max-w-3xl",
    "max-w-2xl": "max-w-xl 2xl:max-w-2xl",
...
def process_class_str(class_str):
    ...
    for token in tokens:
        ...
        if m:
            prefix = m.group(1)
            val = float(m.group(2))
            if val < 500: # Don't scale massive containers like max-w-[1920px]
...
with open("app/galleri/page.tsx", "w") as f:
